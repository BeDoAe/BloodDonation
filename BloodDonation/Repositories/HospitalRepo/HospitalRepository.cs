...
namespace BloodDonation.Repositories.HospitalRepo
{
    public class HospitalRepository : Repository<Hospital> , IHospitalRepository
    {
        ...
        public async Task<List<Donor>> getAllDonors(string id)
        {
            List<Donor> donors = await context.Donors.Where(h=>h.Id==id).ToListAsync();
            return donors;
        }
        public async Task<List<Reciepent>> getAllReciepents(string id)
        {
        ...
        public async Task<List<DonateBlood>> getAllBlooodDonations()
        {
        ...
        {
            int? AvailableBloodAmount = await context.Hospitals.Where(h => h.Id == id).Select(h => h.TotalBloodAmount).FirstOrDefaultAsync();
